...
std::vector<uint8_t> aes128_encrypt_cbc_openssl(
    std::vector<uint8_t> const &plaintext, const char key[AES_KEY_SIZE]
) {
    AES_KEY aes_enc_key;
    AES_set_encrypt_key(
    ...
    AES_cbc_encrypt(
        reinterpret_cast<const uint8_t *>(paddedPlaintext.data()),
        ciphertext.data(), paddedPlaintext.size(), &aes_enc_key, iv.data(),
        AES_ENCRYPT
    );
...
void set_encrypt_key(__m128i const *key, __m128i *roundKeys) {
    ...
    constexpr uint8_t rcon[10] = {0x01, 0x02, 0x04, 0x08, 0x10,
...
std::vector<uint8_t> aes128_encrypt_cbc_aesni(
    std::vector<uint8_t> const &plaintext, char const key[AES_KEY_SIZE]
) {
    // generate round keys
